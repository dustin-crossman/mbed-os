...
# Arguments:
# 1: local asset directory, relative to ROOT_DIR (for example: output/psoc6pdl)
...
function fetch_asset()
{
    ...
    local asset_path="$ROOT_DIR/$asset_dir"
    local asset_timestamp="$asset_path/.timestamp"

    # Never remove existing git clone
    if [[ -e "$asset_path/.git" ]]; then
    ...
    if [[ -d "$asset_path" ]] && ! curl -sSf "$ASSET_BASEURL" >/dev/null 2>&1; then
        echo "Skip $asset_dir download: $ASSET_BASEURL is not accessible"
        return
    fi

    if [[ "${asset_zip}" =~ (.*)([0-9]+\.[0-9]+\.[0-9]+)\.Latest(.*) ]]; then
    ...
    elif [[ "${asset_zip}" =~ (.*)([0-9]+\.[0-9]+\.[0-9]+)\.([0-9]+) ]]; then
    ...
    echo "Extracting $asset_zip to $asset_path"
    unzip -qbo "$OUT_DIR/$asset_zip" -d "$extract_dir"
...
function fetch_psoc6pdl()
{
    ...
    local pdl_build=${2:-"1.20.0.Latest"}
    local pdl_zip="psoc6pdl-${pdl_build}.zip"
    local cm0p_zip="psoc6cm0p-${pdl_build}.zip"

    fetch_asset "output/psoc6pdl" "repo/psoc6pdl" "$pdl_branch" "$pdl_zip" "$OUT_DIR"
    fetch_asset "output/psoc6cm0p" "repo/psoc6pdl" "$pdl_branch" "$cm0p_zip" "$OUT_DIR"
}
...
{
    rm -rf "$OUT_DIR/whd"
    fetch_asset "output/whd" "repo/whd" "master" "whd.zip" "$OUT_DIR/whd"
}

#This function downloads capsense and ble middleware
function fetch_middleware()
{
    fetch_asset "output/capsense" "repo/middleware-capsense" "mt2p0" "middleware-capsense-2.0.0.Latest.zip" "$OUT_DIR"
    fetch_asset "output/ble" "repo/middleware-bless" "mt2p0" "middleware-bless-3.20.0.Latest.zip" "$OUT_DIR"
}
...
function extract_artifacts()
{
    ...
    if [[ ! -d "$CSP_TOP_DIR" ]]; then
        ...
        unzip -qbo "$csp_zip" -d "$OUT_DIR"
    fi

    # Extract BSP assets
    for bsp_zip in "$BSP_TOP_DIR"/*-$PKG_VERSION.zip; do
        unzip -qbo "$bsp_zip" -d "$BSP_TOP_DIR"
    done
