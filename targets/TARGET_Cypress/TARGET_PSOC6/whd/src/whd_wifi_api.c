...
******************************************************/
static /*@null@*/ void *whd_wifi_join_events_handler(whd_interface_t ifp, const whd_event_header_t *event_header,
...
static uint32_t whd_wifi_prepare_join(whd_interface_t ifp,
                                      ...
                                      /*@unique@*/ const uint8_t *security_key,
                                      uint8_t key_length,
                                          ...
                                          /*@returned@*/ void *handler_user_data)
{
...
#if 0
    if ( ( (uint32_t)event_header->interface != WHD_STA_INTERFACE ) &&
         ( (uint32_t)event_header->interface != WHD_P2P_INTERFACE ) )
    {
        // WPRINT_WHD_ERROR(("%s: Bad event header interface, 0x%x\n", __FUNCTION__, event_header->interface));
        ...
        case WLC_E_PSK_SUP:
            ...
            if ( (whd_driver->internal_info.whd_join_status[event_header->bsscfgidx] & JOIN_LINK_READY) != 0 )
            {
                ...
                else
                {
                    ...
                    {
                        WPRINT_WHD_DEBUG( ("Unsuccessful supplicant event; status=0x%x\n", event_header->status) );
    ...
    if (join_attempt_complete == WHD_TRUE)
    {
        if (semaphore != NULL)
        {
            whd_rtos_get_semaphore(&whd_driver->internal_info.active_join_mutex, NEVER_TIMEOUT, WHD_FALSE);
            if (whd_driver->internal_info.active_join_semaphore != NULL)
            {
                ...
                whd_rtos_set_semaphore(whd_driver->internal_info.active_join_semaphore, WHD_FALSE);
            }
            whd_rtos_set_semaphore(&whd_driver->internal_info.active_join_mutex, WHD_FALSE);
        }
        return NULL;
    }
    else
...
static uint32_t whd_wifi_prepare_join(whd_interface_t ifp, whd_security_t auth_type,
                                      /*@unique@*/ const uint8_t *security_key, uint8_t key_length,
                                      whd_semaphore_type_t *semaphore)
{
    whd_buffer_t buffer;
...
static void whd_wifi_active_join_deinit(whd_interface_t ifp, whd_semaphore_type_t *stack_semaphore, whd_result_t result)
{
    ...
    {
        WPRINT_WHD_INFO( ("Failed join (err %d)\n", result) );
    }
...
static uint32_t whd_wifi_join_wait_for_complete(whd_interface_t ifp, whd_semaphore_type_t *semaphore)
{
    ...
    while (!done)
    {
        ...
        done = ( (whd_rtos_get_time( ) - start_time) >= DEFAULT_JOIN_ATTEMPT_TIMEOUT );
    }

    if (result != WHD_SUCCESS)
    {
        CHECK_RETURN(whd_wifi_leave(ifp) );
        WPRINT_WHD_INFO( ("%s: not ready to transceive (err %d); left network\n", __func__, result) );
    }
                                ...
                                uint8_t key_length)
{
    ...
    whd_driver_t whd_driver            = ifp->whd_driver;
    whd_semaphore_type_t *semaphore = NULL;
...
/*    if ( whd_wifi_set_block_ack_window_size( ifp ) != WHD_SUCCESS )
    {
        WHD_WLAN_LET_SLEEP( );
        WPRINT_WHD_INFO( ( "%s: ba set failed\n", __FUNCTION__ ) );
    ...
    {
        result = whd_wifi_active_join_init(ifp, security, security_key, key_length, semaphore);
    }
    ...
    if (result == WHD_SUCCESS)
    {
        ...
        if ( (ifp->role == WHD_AP_ROLE) && (whd_wifi_is_ready_to_transceive(ifp) == WHD_SUCCESS) )
        {
            ...
            if (current_softap_channel != ap->channel)
            {
                ...
                WPRINT_WHD_DEBUG( ("WARN: moving soft-AP channel from %lu to %d due to STA join\n",
        ...
        WPRINT_WHD_INFO( ("%s: set_ssid result (err %d); left network\n", __func__, result) );
        ...
        {
            WPRINT_WHD_INFO( ("%s:3 not ready to transceive (err %d); left network\n", __func__, result) );
        }
    }
    else
    {
        WPRINT_WHD_INFO( ("%s: active join init failed: (%d)\n", __FUNCTION__, result) );
    }
...
/*
    if ( whd_wifi_set_block_ack_window_size( ifp ) != WHD_SUCCESS )
    {
        WPRINT_WHD_ERROR( ( "%s: failed to set ba win\n", __func__ ) );
        WHD_WLAN_LET_SLEEP( );
        return WHD_SET_BLOCK_ACK_WINDOW_FAIL;
    }
 */
...
uint32_t whd_wifi_leave(whd_interface_t ifp)
{
    ...
    CHECK_RETURN(whd_management_set_event_handler(ifp, join_events, NULL, NULL) );
    ...
    if (result != WHD_SUCCESS)
    {
        WPRINT_WHD_DEBUG( ("whd_sdpcm_send_ioctl(WLC_DISASSOC) failed:%d\r\n", result) );
    }
                                          ...
                                          /*@returned@*/ void *handler_user_data)
{
    ...
    if (event_header->event_type == WLC_E_PFN_NET_FOUND)
    {
        if (whd_driver->internal_info.whd_scan_result_ptr != NULL)
        {
            ...
            /* Firmware needs to be patched to allow returning the bss info in the result,
               if we want full scan results from PNO.
               As the supported use case doesn't require that,
               leaving as-is to reduce memory usage here and in FW.
             */
            ...
            record->bss_type = WHD_BSS_TYPE_INFRASTRUCTURE;
            record->channel = pfn_subnet->channel;
            record->band =
                (whd_channel_to_wl_band(whd_driver,
                                        record->channel) ==
                 GET_C_VAR(whd_driver, CHANSPEC_BAND_2G) ) ? WHD_802_11_BAND_2_4GHZ : WHD_802_11_BAND_5GHZ;

            /* For PNO, the user has already specified security to look for; whd layer will update this from recorded data */
            record->security = WHD_SECURITY_UNKNOWN;
        }
    ...
    version = WHD_READ_32(&bss_info->version);
    ...
    {
        WPRINT_WHD_ERROR( ("Invalid bss length check %s: SSID_len:%d,ie_len:%ld,ie_off:%d\n", __FUNCTION__,
    ...
    record->signal_strength = ( int16_t )(WHD_READ_16(&bss_info->RSSI) );
    ...
    chanspec = WHD_READ_16(&bss_info->chanspec);
    if (bss_info->n_cap)
    {
                       ...
                       )
{
    ...
    if ( (channel_list_size > 0) && (optional_channel_list != NULL) )
    {
        ...
        scan_params->params.channel_num = htod32( (int32_t)channel_list_size );
    }
...
uint32_t  whd_wifi_deauth_sta(whd_interface_t ifp, whd_mac_t *mac, whd_dot11_reason_code_t reason)
{
    ...
    if (mac == NULL)
    {
        ...
        CHECK_RETURN(whd_wifi_get_ap_info(ifp, &ap_info, &sec) );
...
#if 0
    /* Handle Splint bug *//*@-noeffect@*/ (void)whd_wifi_ap_is_up; /*@+noeffect@*/
#endif
...
uint32_t whd_wifi_manage_custom_ie(whd_interface_t ifp, whd_custom_ie_action_t action, /*@unique@*/ const uint8_t *oui,
...
{
    whd_result_t result = whd_wifi_get_iovar_buffer(ifp, IOVAR_STR_VERSION, (uint8_t *)buf, length);
...
uint32_t whd_network_get_ifp_from_ifidx(whd_interface_t *ifp, uint8_t ifidx)
{
    //To be implemented
    return WHD_SUCCESS;
}
