        ...
        cy_func_get_random_data_t GetRandomDataFunc, void *randomDataInfo)
{
    ...
    if ((eccDp != NULL) && (key != NULL))
    {
        ...
        if (GetRandomDataFunc != NULL)
        {
            ...
            Cy_Crypto_Core_Vu_SetMemValue(base, VR_D, (uint8_t *)key, bitsize);
        }
        else
        {
            uint32_t i = 0U;
            ...
            uint32_t *tmpRegPtr = Cy_Crypto_Core_Vu_RegMemPointer(base, p_key);
            ...
            Cy_Crypto_Core_InvertEndianness(tmpRegPtr, bytesize);

            /* Shift key right */
            CY_CRYPTO_VU_SET_REG(base, p_temp, bytesize * 8u - bitsize, 1u);
            CY_CRYPTO_VU_LSR(base, VR_D, p_key, p_temp);

            CY_CRYPTO_VU_FREE_MEM(base, CY_CRYPTO_VU_REG_BIT(p_key));
        }
        ...
        if (CY_CRYPTO_SUCCESS == tmpResult)
        {
            ...
            CY_CRYPTO_VU_CMP_SUB (base, VR_D, VR_P);                    /* C = (a >= b) */
        ...
        CY_CRYPTO_VU_FREE_MEM(base, CY_CRYPTO_VU_REG_BIT(VR_D));
    }
