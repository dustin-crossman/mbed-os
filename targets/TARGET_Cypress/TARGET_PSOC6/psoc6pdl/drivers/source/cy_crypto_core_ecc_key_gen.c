    ...
    if ((eccDp != NULL) && (key != NULL))
    {
        ...
        ***************************************************************/
        ...
        else
        {
            ...
            cy_en_crypto_status_t error = CY_CRYPTO_SUCCESS;

            while ((randomsize > 0u) && (CY_CRYPTO_SUCCESS == error))
            {
                uint32_t randombits = (uint32_t)CY_CRYPTO_MIN(randomsize, 32);

                error = Cy_Crypto_Core_Trng(base, CY_ECC_CONFIG_TR_GARO_CTL, CY_ECC_CONFIG_TR_FIRO_CTL,
        ...
        cy_func_get_random_data_t GetRandomDataFunc, void *randomDataInfo)
{
    ...
    if ((eccDp != NULL) && (key != NULL))
    {
        ...
        else
        {
            ...
            for (int32_t i = 0, randomsize = bitsize; (randomsize > 0) && (CY_CRYPTO_SUCCESS == error); randomsize-=32, i++)
            {
                uint32_t randombits = (uint32_t)CY_CRYPTO_MIN(randomsize, 32);
