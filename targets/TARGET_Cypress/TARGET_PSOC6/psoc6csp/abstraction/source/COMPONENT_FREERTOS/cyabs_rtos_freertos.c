    ...
    const char *name, void *stack, uint32_t stack_size, cy_thread_priority_t priority, cy_thread_arg_t arg)
{
    ...
    {
        last_error = xTaskCreate((TaskFunction_t)entry_function, name, stack_size, (void *)arg, priority, thread);
...
cy_rslt_t cy_rtos_get_mutex(cy_mutex_t *mutex, cy_time_t timeout_ms)
{
    ...
    else
    {
        ...
        if (xSemaphoreTake(*mutex, ticks) == pdFALSE)
    ...
    cy_timer_callback_t fun, cy_timer_callback_arg_t arg)
{
    ...
    else
    {
        ...
        *timer = xTimerCreate("", 0, reload, arg, fun);
...
cy_rslt_t cy_rtos_get_time(cy_time_t *tval)
{
    ...
    {
        *tval = (xTaskGetTickCount() * 1000) / configTICK_RATE_HZ;
        status = CY_RSLT_SUCCESS;
    }
    return status;
}
...
{
    vTaskDelay((num_ms * configTICK_RATE_HZ) / 1000);
    return CY_RSLT_SUCCESS;
}
