...
#define GET_ARRAY_INDEX(block, channel)  (cy_internal_data[block].channel_offset + channel)
...
static cy_timer_irq_handler callbacks[TCPWM_CHANNELS];
...
typedef struct {
    ...
    uint8_t isr_offset;
} cy_internal_timer_data_t;

static const cy_internal_timer_data_t cy_internal_data[] = {
#if (CY_IP_MXTCPWM_INSTANCES > 0)
...
{
    TCPWM_Type *blockAddr = cy_internal_data[block].base;
    uint32_t index = GET_ARRAY_INDEX(block, channel);

    if (callbacks[index] != NULL)
    {
        /* Call registered callbacks here */
        (void)(callbacks[index])(callback_args[index], (cy_timer_irq_event_t)Cy_TCPWM_GetInterruptStatus(blockAddr, channel));
    }
...
static inline uint32_t convert_direction(cy_timer_direction_t direction)
{
    switch (direction)
    {
        case CY_TIMER_DIR_UP:
            return CY_TCPWM_COUNTER_COUNT_UP;
        case CY_TIMER_DIR_DOWN:
            return CY_TCPWM_COUNTER_COUNT_DOWN;
        case CY_TIMER_DIR_UP_DOWN:
...
{
    if (NULL == obj)
        return CYCSP_TIMER_RSLT_BAD_ARGUMENT;
    //TODO: Handle Trigger mux pin assignments
    if (CY_NC_PIN_VALUE != pin)
        return CYCSP_TIMER_RSLT_BAD_ARGUMENT;

    memset(obj, 0, sizeof(cy_timer_t));
    obj->pin = pin;
    obj->clock.div_num = CY_RSC_INVALID;

    cy_rslt_t result = cy_hwmgr_allocate(CY_RSC_TCPWM, &obj->resource);
    if (CY_RSLT_SUCCESS == result)
    {
        cy_resource_inst_t *timer = &obj->resource;
        obj->base = cy_internal_data[timer->block_num].base;

        en_clk_dst_t pclk = (en_clk_dst_t)(cy_internal_data[timer->block_num].clock_dst + timer->channel_num);
        ...
        if (NULL != clk)
        {
            ...
            if (CY_SYSCLK_SUCCESS != Cy_SysClk_PeriphAssignDivider(pclk, clk->div_type, clk->div_num))
                result = CYCSP_TIMER_RSLT_FAILED_CLOCK;
        }
            ...
            !configured)
        {
            if (CY_TCPWM_SUCCESS == Cy_TCPWM_Counter_Init(obj->base, obj->resource.channel_num, &default_config))
                ...
                result = CYCSP_TIMER_RSLT_FAILED_INIT;
        }
...
{
    cy_rslt_t rslt = CYCSP_TIMER_RSLT_BAD_ARGUMENT;
    if (NULL != obj && NULL != obj->base)
    {
        rslt = cy_hwmgr_free(&obj->resource);
...
cy_rslt_t cy_timer_register_irq(cy_timer_t *obj, uint8_t priority, cy_timer_irq_handler handler, void *handler_arg)
{
    ...
    IRQn_Type irqn = (IRQn_Type)(cy_internal_data[obj->resource.block_num].isr_offset + obj->resource.channel_num);
...
{
    Cy_TCPWM_SetInterrupt(obj->base, obj->resource.channel_num, (uint32_t)event);
    Cy_TCPWM_SetInterruptMask(obj->base, obj->resource.channel_num, (uint32_t)enable);
