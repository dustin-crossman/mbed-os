...
void cy_multi_adv_internal_cb(uint16_t opcode, uint8_t status, uint16_t data_len, uint8_t *data)
{
    ...
    {
        (*multi_adv_cb)((hci_multi_adv_sub_opcode_t)*data, (uint8_t)status);
    }
}
void CyMulti_adv::cy_register_multi_adv_cmd_complete_cb(CY_multi_adv_cb_t *cb)
{
    multi_adv_cb = cb;
}

void CyMulti_adv::cy_start_multi_advertisements(uint8_t advertising_enable, uint8_t adv_instance)
{
...
ble_error_t CyMulti_adv::cy_set_multi_advertisement_scan_response_data( uint8_t * p_data, uint8_t data_len, uint8_t adv_instance )
{
    return multi_adv_set_data( p_data, data_len, adv_instance, SET_SCAN_RESP_DATA);
}

ble_error_t CyMulti_adv::cy_set_multi_advertisement_data( uint8_t * p_data, uint8_t data_len, uint8_t adv_instance )
{
    return multi_adv_set_data( p_data, data_len, adv_instance, SET_ADVT_DATA);
}

ble_error_t CyMulti_adv::cy_set_multi_advertisement_params( uint8_t adv_instance, const cy_ble_multi_adv_params_t p_param )
{
